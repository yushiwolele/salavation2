...
class ReuseCommonTestCases:
    ...
    def create_project_test_cases(self,file):
        ...
        for row in data_list:
            ...
            if module =='' and test_type =='' and test_flg =='':
                break
            if test_flg != '':
                ...
                if test_type in self.extract_sheetname_gn:  # 前端功能页面
                    ...
                    for data_row in range(1, len(data_gn_list)):  # 获取最新二维数据列表的行数
                        ...
                        # self.modify_testcase_mergename(ws_tag, 12, '''=IF(K%s="自动化",F%s&"-"&G%s,"")''', [0, 0, 0],{'4':'全流程;界面基本功能'})

                        list_modify = [{'modify_col_num':11,'modify_cont': '''=IF(K%s="自动化",E%s,"")''','excur_list':[0, 0],'exclude':{'4': '全流程;界面基本功能'},'modify_start_row':1},
                            {'modify_col_num':12,'modify_cont': '''=IF(K%s="自动化",F%s&"-"&G%s,"")''','excur_list':[0, 0, 0],'exclude':{'4': '全流程;界面基本功能'},'modify_start_row':1}]
                        self.modify_testcase_mergename(ws_tag, list_modify)
                        # =====================================================================================================
                        if not xl.conditional_formatting(testcase_sheetname):  # 设置条件格式
                            return {'code':1,'data':'[%s][%s]sheet页生成有误'%(test_type,testcase_sheetname)}
                # 当“测试点分类”为对外接口服务时
                elif test_type in self.extract_sheetname_jkfw:
                    # 读取“对外接口服务配置”页的接口对象类别(宏、视图、表、存储过程、函数等)
                    if not data_jkfw_list:
                        msg = '文件异常，请检查文档sheet页[%s]是否为最新模板!'%(self.extract_sheetname_jkfw['对外接口服务'])
                        logger.error(msg)
                            ...
                            if data_jkfw_list[0][data_col] == '宏':
                                if data_jkfw_list[data_row][data_col]:
                                    ...
                                    if not xl.conditional_formatting(data_jkfw_list[data_row][data_col].split('.')[1]): #设置条件格式
                                        return {'code':1,'data':'[%s]sheet页生成有误'%(test_type)}
                                elif data_row == 1:
        ...
        return {'code':0,'data':xl.tag_excelbook}
    ...
    def modify_testcase_mergename(self, worksheet, list_modify):
        '''
        调整测试用例中某整列的内容
        :param worksheet: 读取到内存文件中的sheet页(含表头)
        ...
        :return:
        '''
        for modify_item in list_modify:
            modify_start_row = modify_item['modify_start_row']
            exclude = modify_item['exclude']
            excur_list = modify_item['excur_list']
            modify_cont = modify_item['modify_cont']
            modify_col_num = modify_item['modify_col_num']
...
class  ExcelExtractMergeSheet(object):
    ...
    def __init__(self):
        ...
        dxf_font_black = DifferentialStyle(font=Font(italic=False,color='000000')) #字体黑色
    ...
    def read_excel_tag(self,tag_file):
        ...
        '''
        try:
            file = load_workbook(tag_file,keep_vba=True)
            self.tag_excelbook = file
        except Exception as e:
            logger.error('报错信息：' + repr(e) + ',报错的行号:' + str(e.__traceback__.tb_lineno))
            return False
